...
窄视口中，讨论操作位于两个决定按钮上方，使所有操作保持在卡片内可见。
...
Host 启用持久化问题时，所选 Session 的 `userQuestions` 投影在刷新或副本替换后恢复相同卡片。提交将会话、问题标识、版本和回答发往 `session.answerQuestion`，接收失败时卡片仍可回答。导航和插件卸载只移除呈现，不删除已记录问题。
