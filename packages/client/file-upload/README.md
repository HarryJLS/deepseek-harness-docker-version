...
An optional `SharedFileUploadStore` authorizes raw transfers without activating an Agent and retains completed receipts across replicas. Missing or inaccessible Sessions return `session/not-found` before byte storage; storage failures remain errors. Resolution is asynchronous and rechecks the receiving user and physical Session incarnation. Committed prompt delivery records `file-upload/consumed`; another replica cannot reuse that receipt after the Agent is released. The provider owns receipt expiry, while attachment bytes remain outside the receipt store. [Shared receipt operations](../../../docs/subsystems/attachment.md) define provider obligations.
