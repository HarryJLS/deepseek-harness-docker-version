...
可选的 `SharedFileUploadStore` 在不激活 Agent 的情况下授权原始字节传输，并跨副本保留已完成凭证。缺失或无权访问的 Session 在字节存储前返回 `session/not-found`；存储故障仍作为错误报告。解析过程为异步操作，会重新检查接收用户及 Session 物理世代。已提交的 prompt 投递记录 `file-upload/consumed`，因此 Agent 释放后，其他副本不能重用该凭证。提供方负责凭证过期，附件字节保留在凭证存储之外。[共享凭证操作](../../../docs/subsystems/attachment.zh.md)定义提供方义务。
