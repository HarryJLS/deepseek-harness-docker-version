...
A backend may expose `sharedExecution` for replica-safe activation. Consumers acquire its reservation before mutating recovery, hold it through agent quiescence and flush, and then release it. Observers of another replica's live work read the committed prefix through a read handle without adding interruption repairs. The [MySQL provider](../session-persistence-mysql/README.md) implements this optional capability.
