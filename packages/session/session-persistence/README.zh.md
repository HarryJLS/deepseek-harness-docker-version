...
后端可暴露 `sharedExecution` 以支持跨副本安全激活。Consumer 在会修改日志的恢复前取得执行权，持有至 agent 停止且写入完成后再释放。观察其他副本存活工作的调用方通过只读句柄读取已提交前缀，不追加中断修复。[MySQL 提供方](../session-persistence-mysql/README.zh.md) 实现此可选能力。
