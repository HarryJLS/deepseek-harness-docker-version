...
export class SessionCommandController {
  ...
  async fork(request: SessionForkRequest, reservedChildId?: SessionId): Promise<SessionForkValue> {
    ...
    try {
      const persistence = this.ctx.get('sessionPersistence')
      if (persistence?.sharedExecution !== undefined) {
        await using reader = await persistence.open(request.sessionId, 'read')
        const stored = await reader.read()
        observed = {
          header: reader.header,
          events: stored.events,
          projections: this.ctx.sessionProjections.restore(
            {}, stored.events, SessionLogOffset(0), reader.header, reader.inheritedEventCount,
          ).snapshot,
          [Symbol.dispose]: () => {},
        }
      } else {
    ...
    const childId = reservedChildId ?? brandString<SessionId>(`session-${randomUUID()}`)
  ...
  async prompt(request: SessionPromptRequest): Promise<SessionPromptValue> {
    ...
    if (pendingQuestion !== null && pendingQuestion !== undefined) {
      throw new RemoteError('session/agent-busy', 'Answer or dismiss the pending question before sending another message.', { reason: 'question-pending' })
    }
    ...
    const admit = async (): Promise<SessionPromptValue> => {
      try {
        ...
        const admission = await resolvePromptFileReceipts(
          request.content,
          receiptId => this.ctx.fileUploads.resolveForRequest(agent, receiptId),
        )
...
async function resolvePromptFileReceipts(
  content: SessionPromptRequest['content'],
  stagedFile: (receiptId: FileUploadReceiptId) => Promise<FileAttachmentRef | undefined>,
): Promise<{ readonly content: AttachmentAdmissionPart[]; readonly receiptIds: readonly FileUploadReceiptId[] }> {
  const receiptIds = new Set<FileUploadReceiptId>()
  const resolved = await Promise.all(content.map(async (part): Promise<AttachmentAdmissionPart> => {
    if (part.type !== 'file') return part
    const attachment = await stagedFile(part.receiptId)
