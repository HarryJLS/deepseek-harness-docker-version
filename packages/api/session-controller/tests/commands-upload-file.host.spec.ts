...
import type { FileUploadGeneration, SharedFileUploadStore } from '@deepseek-ai/dsh-client-file-upload'
...
describe('Session file uploads', () => {
  it('uploads without a live Agent and consumes the receipt on a replacement replica only once', async () => {
    const rows = new Map<FileUploadReceiptId, FileAttachmentRef>()
    const shared: SharedFileUploadStore = {
      authorize: async () => 'row-1' as FileUploadGeneration,
      save: async (_id, _generation, receiptId, file) => { rows.set(receiptId, structuredClone(file)) },
      read: async (_id, receiptId) => rows.get(receiptId),
    }
    const first = await uploadHarness()
    const second = await uploadHarness()
    const third = await uploadHarness()
    try {
      first.uploads.registerSharedStore(shared)
      second.uploads.registerSharedStore(shared)
      third.uploads.registerSharedStore(shared)
      first.disposeAgent()
      const receipt = await first.uploads.uploadStream({
        sessionId: SESSION,
        data: (async function* () { yield new Uint8Array([1, 2, 3]) })(),
        name: 'shared.txt',
      })
      await first.ctx.fiber.dispose()
      expect(second.uploads.resolve(second.agent, receipt.receiptId)).toBeUndefined()
      const request = promptRequest([{ type: 'file', receiptId: receipt.receiptId }])
      await second.controller.prompt(request)
      expect(second.followup).toHaveBeenCalledOnce()
      expect((second.followup.mock.calls[0]?.[0] as UserMessage).content)
        .toEqual([{ type: 'file', attachment: receipt.file }])
      const consumed = second.agent.session.snapshotEvents().find(event => event.type === 'file-upload/consumed')
      expect(consumed).toMatchObject({ data: { requestId: 'req-1', receiptIds: [receipt.receiptId] } })
      if (consumed?.type !== 'file-upload/consumed') throw new Error('receipt consumption was not recorded')
      third.agent.session.append(consumed.type, consumed.data)
      await expect(third.controller.prompt({ ...request, requestId: 'different-request' as SessionRequestId }))
        .rejects.toMatchObject({ code: 'session/attachment-invalid' })
      expect(third.followup).not.toHaveBeenCalled()
    } finally {
      await Promise.all([first, second, third].map(value => value.ctx.fiber.dispose()))
    }
  })

  it('authorizes shared uploads before consuming bytes and withdraws shared storage on disposal', async () => {
    const fixture = await uploadHarness()
    ...
    try {
      const dispose = fixture.uploads.registerSharedStore(shared)
      expect(() => fixture.uploads.registerSharedStore(shared)).toThrow('already registered')
      await expect(fixture.uploads.uploadStream({
        sessionId: SESSION,
        data: (async function* () { consumed(); yield new Uint8Array([1]) })(),
      })).rejects.toMatchObject({ code: 'session/not-found', details: { sessionId: SESSION } })
      expect(consumed).not.toHaveBeenCalled()
      await expect(fixture.uploads.upload(fixture.agent, { data: 'AAAA' }, new AbortController().signal))
        .rejects.toMatchObject({ code: 'session/not-found' })
      expect(fixture.saveFile).not.toHaveBeenCalled()
      expect(save).not.toHaveBeenCalled()
      dispose()
      fixture.uploads.registerSharedStore(shared)
      dispose()
      expect(() => fixture.uploads.registerSharedStore(shared)).toThrow('already registered')
    } finally {
  ...
  it.each([
    { name: 'an inaccessible Session', failure: undefined, code: 'session/not-found' },
    { name: 'a failed receipt database', failure: new Error('receipt database unavailable'), code: 'gateway/internal' },
  ])('reports $name through the raw upload route without storing bytes', async ({ failure, code }) => {
    const fixture = await uploadHarness()
    const save = vi.fn()
    const shared: SharedFileUploadStore = {
      authorize: async () => {
    ...
    try {
      fixture.uploads.registerSharedStore(shared)
      const response = await fixture.uploadRoute(new Request(`http://host/upload?sessionId=${SESSION}`, {
        method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: 'private file',
      }))
      expect(response.status).toBe(200)
      expect(await response.json()).toMatchObject({ ok: false, error: { code } })
      expect(fixture.saveFileStream).not.toHaveBeenCalled()
      expect(save).not.toHaveBeenCalled()
