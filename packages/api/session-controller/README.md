...
When the persistence provider supplies shared execution, Web mutations activate a fresh, exclusively reserved Agent and release it after its work is durable. `answerQuestion` accepts a recorded question identity/version rather than a process callback. Shared `follow` streams read committed events and supply `state` frames for projections and running status; they never activate an Agent merely to observe a remote writer. The process-local control feed is empty in this mode. See [deployment requirements](../../../deploy/README.md#shared-confirmation).

Shared history rebuilds a stale projection checkpoint from the complete durable log when a preset registers another projection unit. Unit removal also updates the client baseline without waiting for a new event.

An optional shared Assistant store publishes cumulative transient snapshots under the executing replica's reservation. Other replicas reconstruct the opening baseline and dense start, chunk, and end frames; committed settlements recover missed transient chunks. A superseded reservation cannot publish into its successor's stream.
