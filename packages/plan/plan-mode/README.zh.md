...
`plan` 投影折叠 `plan/mode` 事件和持久化问题的批准决定。没有轮次开启时，模式变更会立即追加；轮次开启期间，它保持待生效，直到下一个被接受的轮内 pre-step，且追加失败不能阻塞轮次。`set`/`get` 服务方法及其确切返回状态见 [`src/index.ts`](src/index.ts)；投影注册表或 key 缺失时，第一次依赖它们的访问会显式失败。
