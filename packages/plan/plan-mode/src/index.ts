 ...
 * state.
 *
 * The `plan` projection folds mode events and approved durable reviews, so
 * resume and fork restore the state. User selections remain pending until the next accepted in-turn
...
export const planProjectionDefinition = {
  ...
  apply: (state, event) => {
    ...
    if (event.type === 'user-questions/state'
      ...
      return { ...state, active: false, wanted: null, running: null }
    }
