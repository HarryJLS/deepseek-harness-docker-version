...
The `plan` projection folds `plan/mode` events and approved durable question decisions. A mode change appends immediately when no turn is open; during an open turn it stays pending until the next accepted in-turn pre-step, and an append failure cannot block the turn. The `set`/`get` service methods and their exact return states live in [`src/index.ts`](src/index.ts); the first dependent access fails explicitly when the projection registry or key is absent.
