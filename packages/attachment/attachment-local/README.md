...
Setting `temporaryRoot` explicitly selects temporary storage for both images and general files. References retain a relative path, filename, and verified metadata, never encoded file bytes. Ordinary and streaming file uploads use the same per-user directory policy as images; a supplied relative path must match that user's stored object. The [container deployment](../../../deploy/README.md#temporary-files) uses this mode and does not store attachments in Redis or MySQL.
