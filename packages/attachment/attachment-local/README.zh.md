...
显式设置 `temporaryRoot` 则改用临时存储。引用保留相对路径、文件名和已验证的图片元数据，不保存文件编码字节。目录下的文件按平台用户隔离，路径不能越界或授权读取其他用户的文件。[容器部署](../../../deploy/README.zh.md#temporary-files) 使用此模式，不将附件保存在 Redis 或 MySQL 中。
