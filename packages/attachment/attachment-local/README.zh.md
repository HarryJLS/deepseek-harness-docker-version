...
| `temporaryRoot` | 无 | 显式选择服务工作目录的相对子目录下按用户隔离的非持久文件 |
...
显式设置 `temporaryRoot` 会为图片和通用文件选择临时存储。引用保留相对路径、文件名和已验证的元数据，不保存文件编码字节。普通及流式文件上传与图片采用相同的按用户分目录策略，所提供的相对路径必须匹配该用户的存储对象。[容器部署](../../../deploy/README.zh.md#temporary-files) 使用此模式，不将附件保存在 Redis 或 MySQL 中。

运维操作或容器生命周期可以删除临时文件。Agent 执行下一步前，输入消息与已有模型历史中的缺失图片转为纯路径文本；已有历史的替换追加到日志中，因此恢复能重建相同的省略结果。仍存在的文件可以继续参与图片请求。文件损坏与权限错误仍报错，已开始的请求若与文件清理并发，仍可能失败。
...
- **临时存储需要显式启用**——不承诺文件保留，也不自动备份；跨节点访问图片需要运维提供共享临时目录或重新附加文件。
