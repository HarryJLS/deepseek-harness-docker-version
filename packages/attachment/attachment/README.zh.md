...
你可以把图片附加到提示词和命令中；默认 `dsh` 组合会在处理消息前规范化并持久保存图片。会话事件携带引用和已验证的元数据，不包含图片字节、浏览器对象 URL 或提供方 URL。支持光栅格式（PNG、JPEG、WebP、GIF），未发送草稿留在浏览器中。部署可以显式选择[临时本地存储](../attachment-local/README.zh.md#temporary-storage)，其引用额外保留相对路径，但不要求字节在重启后继续存在。
