...
把图片与通用文件附加到提示词和命令中。图片会在消息被接受前完成校验与规范化；部署限额内支持 PNG、JPEG、WebP 和 GIF。其他文件按字节原样保存，不设格式与大小限制；模型通过保存的只读路径按需读取，而不接收文件字节。会话事件不包含浏览器路径、提供方 URL 和 base64。默认存储在重启后保留附件，且从不自动删除；[临时本地存储](../attachment-local/README.zh.md#temporary-storage) 显式允许文件丢失，并记录相对路径。音频和视频暂无专门处理。
