...
describe('web e2e: agent-preset selection', () => {
  ...
  it('labels a resumed session with the preset it was created under', async () => {
    onTestFailed(async () => {
      await saveFailureShot(page, 'web-e2e-agent-preset-header')
      await writeFile(join('.artifacts', 'web-e2e-agent-preset-header.aria.txt'), await page.locator('body').ariaSnapshot())
    })
    // The seeded parent belongs to Ungrouped, not the connected workspace.
    const group = page.getByRole('treeitem', { name: /^Ungrouped/ })
    if (await group.getAttribute('aria-expanded') === 'false') await group.click()
    // Only the persisted parent has row actions; the blank Session has none.
    await page.getByRole('treeitem').filter({
