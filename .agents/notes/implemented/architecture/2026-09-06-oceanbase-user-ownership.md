...
Request ingress and data readers resolve the same `dsh-user-context` peer instance. Its `AsyncLocalStorage` is module-local; another installed copy sees an unscoped operation, which is reserved for trusted Host maintenance. The dependency policy rejects private dependency copies for managed Host consumers, and the SQL, attachment, workspace, and query providers also declare the shared peer.
...
Focused tests cover concurrent user scopes, shared-peer dependency enforcement, live and cold denials, stream filtering, invalid Nacos fields, shell-safe secret transport, Snowflake exhaustion, and OceanBase audit writes, ownership checks, soft deletion, and transaction rollback. JSONL headers and the disposable SQLite query index preserve the same optional ownership metadata; incompatible SQLite layouts are rejected.
