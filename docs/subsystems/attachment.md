...
The attachment seam separates binary image and generic-file ownership from the session log. A producer gives bytes to [`ctx.attachments`](#ctxattachments--attachmentstore-abstract-seam); the service publishes a content-addressed reference after writing the object. Session events and model-visible attachment blocks contain references and metadata, never browser object URLs, provider URLs, or base64 payloads. Durable storage is the default; [temporary local storage](../../packages/attachment/attachment-local/README.md#temporary-storage) explicitly permits file loss and records relative paths. The independent [`ctx.fileUploads`](#ctxfileuploads--fileuploads) service binds browser file transfers and staged receipts to the authorized receiving Session.

Unsent browser drafts may stay in memory and native clients may stage them in operating-system temporary storage. Browser generic files are written before they receive a staged prompt receipt. In the default deployment, the host writes accepted images below `<DSH_HOME>/attachments/v1` before appending the user event. Structured model image output follows the same file-publication-before-event rule. Docker deployments keep images and general files in the configured user-scoped temporary directory.

Source: [`packages/attachment/attachment/src/types.ts`](../../packages/attachment/attachment/src/types.ts)

[`SharedFileUploadStore`](../../packages/client/file-upload/src/shared.ts) optionally shares completed receipts across replicas. `authorize()` checks the receiving Session before consuming bytes and returns its branded `FileUploadGeneration`, or `undefined` for a missing or inaccessible Session; other storage failures reject. `save()` binds that physical incarnation and the authorized user to an immutable receipt, and `read()` rechecks both before returning an unexpired reference. Accepted receipts also enter the Session log as `file-upload/consumed`, preventing reuse after process replacement. Binary bytes remain in the attachment provider.
