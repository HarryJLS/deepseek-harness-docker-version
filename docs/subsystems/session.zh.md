...
[`SharedAssistantStateStore`](../../packages/api/session-controller/src/shared-assistant.ts) 在不激活 Agent 的情况下读取当前执行权所有者的累积 Assistant 流。其 `createWriter()` 将本地执行权捕获到 `SharedAssistantWriter` 中；该执行权过期或被替换后，`publish()` 必须拒绝。共享传输不携带附件字节，也不替代持久 Session 日志。
