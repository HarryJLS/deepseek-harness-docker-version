...
[`SharedAssistantStateStore`](../../packages/api/session-controller/src/shared-assistant.ts) reads the current reservation owner's cumulative Assistant stream without activating an Agent. Its `createWriter()` captures the local reservation in a `SharedAssistantWriter`; `publish()` must reject after that reservation expires or is replaced. The shared transport carries no attachment bytes and does not replace the durable Session log.
...
/**
 * Install cross-replica Assistant state alongside shared Session persistence.
 * @param store - reservation-bound transient stream storage.
 * @returns disposer removing the installed provider.
 */
