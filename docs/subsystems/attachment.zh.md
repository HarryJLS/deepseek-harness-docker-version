...
附件 seam 将二进制图片和通用文件的所有权与会话日志分离。生产方把字节交给 [`ctx.attachments`](#ctxattachments--attachmentstore-abstract-seam)，服务写入对象后发布内容寻址引用。会话事件和模型可见的附件块包含引用与元数据，不包含浏览器对象 URL、提供方 URL 或 base64 数据。默认使用持久化存储；[临时本地存储](../../packages/attachment/attachment-local/README.zh.md#temporary-storage)显式允许文件丢失，并记录相对路径。独立的 [`ctx.fileUploads`](#ctxfileuploads--fileuploads) 服务将浏览器文件传输及暂存凭证绑定到通过授权的接收 Session。

未发送的浏览器草稿可以保留在内存中，原生客户端也可以将其暂存于操作系统临时存储。浏览器通用文件先写入，再取得暂存 prompt 凭证。默认部署在宿主接受用户消息时，先把图片写到 `<DSH_HOME>/attachments/v1`，再追加用户事件。结构化模型图片输出遵循相同的先发布文件、后记录事件规则。Docker 部署将图片和通用文件保留在配置的按用户隔离的临时目录中。

来源：[`packages/attachment/attachment/src/types.ts`](../../packages/attachment/attachment/src/types.ts)

[`SharedFileUploadStore`](../../packages/client/file-upload/src/shared.ts) 可选地在副本间共享已完成的上传凭证。`authorize()` 在读取字节前检查接收 Session，并返回带类型标记的 `FileUploadGeneration`，对于缺失或无权访问的 Session 则返回 `undefined`；其他存储故障会拒绝操作。`save()` 将该物理世代及已授权用户绑定到不可变凭证，`read()` 则在返回未过期引用前重新检查两者。已接受的凭证也作为 `file-upload/consumed` 写入 Session 日志，防止进程替换后重用。二进制字节仍由附件提供方保存。
