...
`SessionPersistence.sharedExecution` 可选地提供 `SharedSessionExecution` 及其可续期 `SessionExecutionLease`（[类型](../../packages/session/session-persistence/src/execution.ts)）。共享 Consumer 在激活或会修改日志的恢复前取得执行权，确认接收前持久化已准入输入，并在 Agent 停止且最后写入完成后释放。观察者使用只读句柄，不得修复其他副本正在执行的轮次。MySQL 提供方根据数据库时间校验写入方的执行权。

每次日志读写都通过句柄，写入要求句柄拥有独占执行权。读取返回调用方独占的外层 slice，以及生产者建立的事件值别名状态。一种句柄类型服务两种访问，在 `read` 句柄上修改会抛出 `SessionReadOnlyError`。已有活跃写入方时，进程内单写者规则使第二次 `open(id, 'write')` 以 `SessionAlreadyOwnedError` 拒绝。
