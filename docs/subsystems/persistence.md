...
`SessionPersistence.sharedExecution` optionally exposes `SharedSessionExecution` and its renewable `SessionExecutionLease` ([types](../../packages/session/session-persistence/src/execution.ts)). A shared consumer reserves before activation or mutating recovery, flushes its admitted input before acknowledgement, and releases after Agent quiescence and final writes. An observer uses a read handle and must not repair a turn that another replica is executing. The MySQL provider fences writes against database-time ownership.

Every log read and write flows through a handle, and writes require its exclusive ownership. A read returns a caller-owned outer slice and the producer-established aliasing state of its event values. One handle type serves both accesses; a mutation on a `read` handle raises `SessionReadOnlyError`. In-process single-writer ownership makes a second `open(id, 'write')` reject with `SessionAlreadyOwnedError` while an owner is active.
...
interface SessionHeader {
  ...
  /** Platform user owning this session; absent user information belongs to `-`. */
