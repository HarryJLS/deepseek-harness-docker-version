...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a manual standby', () => {
    ...
    const wineAptCache = workflowJob(masterWorkflow, 'wine-apt-cache')
  ...
  it('runs reference suites only after an explicit manual selection', () => {
    ...
    expect(workflowEvent(workflow, 'workflow_dispatch')).toMatchObject({
      inputs: {
        suite: {
          ...
          options: ['wine-apt-cache', 'platform-runtimes', 'self-hosted-standby', 'larger-runner-benchmark', 'consolidated-runner-benchmark'],
        },
      },
    })
    ...
    expect(Object.fromEntries(Object.entries(workflow.jobs).map(([name, job]) => [
      ...
      'python-runtime': "github.event_name == 'workflow_dispatch' && inputs.suite == 'platform-runtimes'",
      windows: "github.event_name == 'workflow_dispatch' && inputs.suite == 'platform-runtimes'",
...
describe('Sandbox workflow', () => {
  ...
  it.skipIf(process.platform === 'win32').each([
    { complete: true, exitCode: 0, accepted: true },
    { complete: true, exitCode: 37, accepted: false },
    { complete: false, exitCode: 0, accepted: false },
  ])('requires both test success and executed files ($complete, $exitCode)', ({ complete, exitCode, accepted }) => {
    const workflow = loadWorkflow('.github/workflows/sandbox.yml')
    const scripts = ['sandbox-e2e', 'seatbelt'].flatMap((name) => {
    ...
    for (const script of scripts) {
      const files = script.includes('packed-install.e2e.ts') ? 1 : 2
      const report = complete ? `Test Files  ${files} passed (${files})` : `Test Files  ${files} skipped (${files})`
      const result = spawnSync('bash', ['--noprofile', '--norc', '-e', '-o', 'pipefail', '-c', [
        'pnpm() { printf \'%s\\n\' "$TEST_REPORT"; return "$TEST_EXIT"; }',
        script.replaceAll('${{ matrix.runner }}', 'landlock'),
      ].join('\n')], {
        encoding: 'utf8',
        timeout: 5000,
        env: { PATH: process.env.PATH, TEST_REPORT: report, TEST_EXIT: String(exitCode) },
      })
      expect(result.error).toBeUndefined()
      expect(result.signal).toBeNull()
      expect(result.status, script).toBe(accepted ? 0 : 1)
    }
  })
})
