...
describe('dependency sections', () => {
  it('rejects a private request-scope dependency and repairs it to one shared peer', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-user-scope-peer-'))
    roots.push(root)
    const userContext = '@deepseek-ai/dsh-user-context'
    const consumer = pkg('@fixture/request-consumer', 'packages/core/request-consumer/package.json', {
      dependencies: { [userContext]: 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^' },
    })
    const source = join(root, consumer.dir, 'src/index.ts')
    mkdirSync(dirname(source), { recursive: true })
    writeFileSync(source, [
      `import { withUser } from '${userContext}'`,
      'export const read = () => withUser(undefined, () => 1)',
      '',
    ].join('\n'))
    const subject = readPackageDependencyFacts(
      root, consumer, 'configured-host', new Set([CORDIS, userContext, consumer.name]), PACKAGE_DEPENDENCY_POLICY,
    )
    const state = { facts: [subject], packages: [], policyViolations: [], workspaceNames: subject.workspaceNames }
    expect(subject.peerRequiredHostDependencies).toEqual(new Set([userContext]))
    expect(collectPackageDependencyViolations(state).join('\n'))
      .toContain(`${userContext} must be matching peerDependencies + devDependencies`)
    repairPackageDependencyManifest(subject)
    expect(subject.manifest.dependencies?.[userContext]).toBeUndefined()
    expect(subject.manifest.peerDependencies?.[userContext]).toBe('workspace:^')
    expect(subject.manifest.devDependencies?.[userContext]).toBe('workspace:^')
    expect(collectPackageDependencyViolations(state)).toEqual([])
  })
